...
export const TodoRTAT = () => {
  ...
  return (
    <div>
      <div role='heading' className='text-lg font-bold'>Todo ReduxToolkit AsyncThunk</div>
      <div className='mb-2 text-purple-500'>AsyncThunk только fetchTodos!</div>
      <div id='todoReduxToolkit' className='w-[350px] border p-1'>
        <TodoForm />
          <div /*TodoList*/ className='h-[250px] overflow-y-scroll'>
          {(status === 'pending' || error) &&
            <div className={`h-full flex items-center justify-center text-center ${error && 'text-sm text-red-500'}`}>
...
const TodoItem = ({todo}) => {
  ...
  return (
    <div className='flex justify-between items-center my-1 gap-x-1 border p-1 hover:bg-zinc-50'>
      <input id={`todoRTAT${todo.id}`} type='checkbox' checked={todo.completed} onChange={() => handlerToggleTodo(todo.id)}className='cursor-pointer' />
